...
def test_reload_is_noop_without_primary_image(qapp):
    fetch_calls = []
    vm = _vm(fetcher=lambda image_id: fetch_calls.append(image_id) or list(SAMPLE_CATEGORIES))
    vm.reload()
    _spin()
    assert fetch_calls == []
...
def test_reload_is_noop_in_multi_selection_mode(qapp):
    ...
    sel = SelectionManager()
    vm = _vm(
        fetcher=lambda image_id: fetch_calls.append(image_id) or list(SAMPLE_CATEGORIES),
        selection_manager=sel,
    )
    sel.select("img-1")
    _wait_for_state(vm, "Ready")
